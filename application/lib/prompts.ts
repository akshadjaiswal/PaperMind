...
OUTPUT FORMAT (JSON):
{
  ...
  "flowchart_mermaid": "Valid Mermaid.js flowchart code. RULES: (1) Start with 'graph TD' (top-down) on its own line — NEVER graph LR. (2) Each node connection on a NEW LINE. (3) Use only --> for plain arrows and -->|label| for labeled arrows. NEVER use -->|label|> or -->> or any other variant. (4) Aim for 6-10 nodes so the chart is readable. EXAMPLE:\ngraph TD\n  A[Research Topic] --> B[Key Concept 1]\n  A --> C[Key Concept 2]\n  B -->|leads to| D[Finding 1]\n  C -->|leads to| E[Finding 2]\n  D --> F[Conclusion]\n  E --> F",
